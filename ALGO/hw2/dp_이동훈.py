...
def main() :
    ...
    for i in range(len(lines)) :
        if i%4 != 1 :
            continue
        arr_list.append([[int(x) for x in lines[i].split(' ')],[int(x) for x in lines[i+1].split(' ')],[int(x) for x in lines[i+2].split(' ')]])

    ans = []
    # 배열 생성
    for k in range(len(arr_list)) :
        max_list = []
        arr = [[None for _ in range(len(arr_list[k][0]))] for _ in range(6)]


        for j in range(len(arr_list[k][0])) :  # 0 ~ 9
            for i in range(6) :  # 0 ~ 5
                arr[i][j] = algo(arr_list[k],arr,i+1,j)
        
        for i in range(6) :
            max_list.append(arr[i][len(arr_list[k][0])-1])
